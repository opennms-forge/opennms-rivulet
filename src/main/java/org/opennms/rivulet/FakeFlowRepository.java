...
public class FakeFlowRepository implements FlowRepository {
    @Override
    public void persist(Collection<Flow> packets, FlowSource source) throws FlowException {
        final Gson gson = new GsonBuilder()
                .serializeNulls()
                .create();

        for (final Flow flow : packets) {
            System.out.println(gson.toJson(FlowDocument.from(flow), FlowDocument.class));
        }
    }
